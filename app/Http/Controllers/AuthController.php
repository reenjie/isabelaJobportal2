...
{   
    ...
    public function signin(Request $request){
       ...
       if(session()->has('countingError')){
        ...
        if($count >=$data['FirstOffensecount']){
            ...
            return redirect()->back()->with('disabled','disable for multiple error login');
    ...
    if(session()->get('countingError') >= $firsterror ){
        return back()->withErrors([
            'email'=>"If you experience two or more consecutive login failures, the system will prohibit you from logging in for your next attempts.",
           ]);
    }else{
        return back()->withErrors([
    ...
    try {
       ...
       $profilesaved =  Profile::create([
        ...
        return back()->withErrors([
            'email'=>"Registration Failed. Email already exist in our database.",
           ]);
    }
